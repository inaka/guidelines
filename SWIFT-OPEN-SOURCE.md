...
*Where should your files go? How should you organize your project's structure? Should an example project go separated… or should it go within your library's repo?*
...
You have a main folder (the one that's opened in the screenshot) whose name must match the repo's name.

Inside that folder, you need to have:

- **The library folder** (in the example: `/Jayme`), which is where your library files will go into. This folder comes by default when you create an Xcode project; however, usually it also contains files like `AppDelegate.swift` or `Main.storyboard`, which are very likely to live among your example project files (normally, you will move them). Just make sure that **only the files you create for your library** go physically under this folder.
...
⚠️ **Before creating a release, don't forget to check that:**
...
If you ever forget  to do something of these and you already created the release, it won't be enough to just delete the release and re-create it; first you will need to delete its associated tag and re-create it, pointing to the new commit where you want your release. Here's a [quick guide](http://stackoverflow.com/a/8044605) on how to do so.
...
Continuous integration in your library is optional. However, is highly recommended that you setup CI if your library is likely to reach a considerable audience. If your library is just a small thing that will have a very limited public and very specific use cases, then we recommend you not to waste time setting up CI.
...
2. Switch the repository you're working with. To do so, you need to have **admin access** to the repo. Ask [Nacho](https://github.com/imendi) for it.
...
4. Add a badge to your `README.md`, which links to the coverage report page of your project. See [Jayme's one](https://github.com/inaka/Jayme/blob/master/README.md) as an example.
